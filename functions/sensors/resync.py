...
def _invoke_worker(ids: list[int], actor: str) -> bool:
    ...
    volver a llamar a /sensors/resync."""
...
def handler(event, context):
    ...
    db = get_db()
    try:
        rows = get_where(db, t("sensors"), where, params, limit=MAX_ROWS)
    except Exception as e:
        return error(500, f"DB error (resync sensors): {e}")
    ...
    return ok({
        "queued": len(ids),
        "batches": batches,
        "message": f"Reintento de sincronización encolado para {len(ids)} sensor(es)",
    })
