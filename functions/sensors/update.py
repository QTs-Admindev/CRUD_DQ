...
def handler(event, context):
    ...
    if payload.get("batch_code") == "":
        payload["batch_code"] = None
    lote_antes = sensor.get("batch_code")
    lote_cambia = "batch_code" in payload and payload["batch_code"] != lote_antes
    ...
    if body.company_id is not None and body.company_id != sensor.get("company_id"):
    ...
    if lote_cambia:
        ...
        try:
            audit(db, event, context, action="update", asset_type="sensor",
                  ...
                  changes={"batch_code": payload["batch_code"]})
        except Exception:
            pass
