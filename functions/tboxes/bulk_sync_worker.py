...
def _sync_one(st, row):
    """Resolve one tbox against the platform. Returns (id, daijin_id | None, error)."""
    ...
    if row.get("daijin_id"):
        try:
            return row["id"], confirm_on_platform(row, "tboxes", client=st), None
    ...
    try:
        daijin_id = resolve_or_create(
            ...
            list_filter={"tboxCode": row["tboxCode"]},
            insert_path="/smartyre/openapi/tbox/insert",
            insert_payload={"tboxCode": row["tboxCode"]},
        )
...
def handler(event, context):
    ...
    # fila marcada a mano quedaría fuera del reintento y /tboxes/resync la encolaría
            ...
            for rid, daijin_id, err in pool.map(lambda r: _sync_one(st, r), chunk):
                ...
                try:
                    ...
                    audit(db, None, context, actor=actor, action="create",
                          asset_type="tbox", asset_id=rid,
                          natural_key=rec.get("tboxCode") if rec else None,
    ...
    if failed_ids:
        ...
        audit(db, None, context, actor=actor, action="create", asset_type="tbox",
