...
- shared/smarttyre/sync.py :: resolve_or_heal — verify a stored daijin_id against
  the platform by natural key, retry a transient empty read across the backoff, and only
...
class FakeST:
    ...
    def get(self, path, params):
        if self.fail:
            raise ConnectionError("the platform down")
...
def test_heal_natural_key_resolves_to_different_id():
    # The asset exists in the platform but under a DIFFERENT id than we stored.
...
def test_heal_phantom_triggers_recreate():
    # Phantom: the stored id never resolves in the platform (empty across all retries).
...
def test_heal_on_resume_swallows_errors_and_rolls_back(monkeypatch):
    updates, audits, db = _wire(monkeypatch, resolve_raises=ConnectionError("the platform down"))
    existing = {"id": 5, "daijin_id": "77", "company_id": 100, "status": "active"}
    ...
    assert "the platform down" in audits[0]["error"]
