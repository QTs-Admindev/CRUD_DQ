...
def resolve_or_heal(st, *, stored_id, list_path, list_filter, insert_path,
                    insert_payload, assume_new=False, backoff=DEFAULT_BACKOFF):
    ...
    if it doesn't resolve, re-creates it, so creation always leaves BOTH systems
    consistent.
    ...
    found = _find_id(st, list_path, list_filter)
    if found is None:
        # Phantom stored_id: the asset is not in Dajin by its natural key. Re-create.
        found = resolve_or_create(
            ...
            assume_new=assume_new, backoff=backoff,
        )
